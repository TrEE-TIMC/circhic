...
def genCircData(HiC,bin_circ=0.5,r_in=0.5,res=0,s_in=800000,s_out=800000,pos0=1,mode='circ',frac_lin=0.7,rotate_lin=0):
    ...
    rotate_lin : float <= 360
        rotation angle (in degree) of the chromosome
        (the linear chromosome is symmetric with respect to the vertical axis by default)
    ...
    #converting indexes of the output matrix into real values (from -1 to 1) such that the size of the matrix is 2 and, hence, the outer radius of the strip is 1
    ...
    #corresponding CLOCKWISE polar angles (Theta); here, we need to consider separately the angles given by acos and asin
    ...
    #default: the CLOCKWISE angle is given by the acos when the asin is negative
    ...
    if mode=='circ': Theta[iR] = (Theta[iR] + np.pi/2)%(2*np.pi)
    elif mode=='lin': Theta[iR] = (Theta[iR] + np.pi/2+frac_lin*np.pi-rotate_lin/360*2*np.pi)%(2*np.pi)
    else: sys.exit('Unknown mode for generating circular data')

    #Bref is the bin "x" of the ms where we consider a contact between x-s/2 and x+s/2
    Bref = np.zeros((Nc,Nc),dtype=int)
    if mode=='circ': Bref[iR] = ((((pos0-1)/Lg+Theta[iR]/2/np.pi)*N).astype(int))%N
    else:
        #with this defition, angles larger than frac_lin*2*pi are irrelevant, which is handled at the very end
    ...
    if mode=='circ':
        ...
        for ic,jc in np.argwhere((iR>0) & (Theta<=frac_lin*2*np.pi) & (Bref - Half_s > 0) & (Bref + Half_s < N)):
...
def plotCirc_simple(Circ,H,colorbar=True):
    ...
    plt.axis('off')
    if colorbar: plt.colorbar(shrink=0.7);
    
    return

def plotCirc(H,r_in,s_in,s_out,genomic_data,marks={},bin_circ=0.5,res=10000,pos0=1,colorbar=True):
    ...
    img=ax_c.imshow(Circ,norm=colors.SymLogNorm(min_non_zero),vmin=vmin,vmax=vmax,cmap='viridis')
    if colorbar: plt.colorbar(img, ax=ax_c, shrink=0.7)
    plt.xticks([]); plt.yticks([]);
    plt.axis('off')
